class AuthService {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.user.type != "referee") {
                    return { success: false, message: `Dieser User ist kein Schiedrichter sondern ${data.user.type}\`.` };
                }
                ...
                if (fullUserData && fullUserData.assignments) {
                    ...
                    document.cookie = `refID=${refID}; path=/; max-age=${7 * 24 * 60 * 60}; secure; samesite=strict`;
                    document.cookie = `authToken=${token}; path=/; max-age=${7 * 24 * 60 * 60}; secure; samesite=strict`;

                    return { success: true, user: fullUserData.user };
                } else {
                    return { success: false, message: 'Failed to fetch team information.' };
                }
            } else {
    ...
    async fetchUserData(userId, token) {
        try {
            ...
            const response = await fetch(`${this.baseURL}/referees/user/${userId}`, { headers });
            if (response.ok) {
